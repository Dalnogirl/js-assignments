...
function extractNameFromTemplate(value) {
  return value.slice(0, value.length - 1).split(' ').slice(1).join(' ')
}
...
function getRectangleString(width, height) {
  ...
  const midAll = `${`│${' '.repeat(width - 2)}│\n`.repeat(height - 2)}`
  const last = `└${'─'.repeat(width - 2)}┘\n`
  return first + midAll + last
}
...
function encodeToRot13(str) {
  return (str + '').replace(/[a-zA-Z]/gi, i => String.fromCharCode(
    i.charCodeAt(0) + (i.toLowerCase() < 'n' ? 13 : -13)))
}
